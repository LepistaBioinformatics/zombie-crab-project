...
## Slice 1 — `crab-reef-network` (new public repository) — DONE
...
- **What:** create the public repo, `go.mod` (module
  `github.com/LepistaBioinformatics/crab-reef-network`, go 1.25, **no requires** — DD-1), dual
...
### T1.5 (done) — `internal/reeflog`: append-only store + reduction
...
- **What:** the endpoints the façade calls, authenticated by `REEF_TOKEN` (HMAC bearer). Publish,
...
### T1.7 (done) — `cmd/crab-reef-network` + Dockerfile
- **What:** boot, env (`REEF_LISTEN`, `REEF_STORE_DIR`, `REEF_TOKEN`), graceful shutdown. Refuse to
  boot with `REEF_TOKEN` unset, naming the variable — the reef is not reachable without a caller
