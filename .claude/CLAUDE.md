...
**The mangrove reaches agents through the MCP server the proxy ALREADY hosts** — a
`mangrove_` namespace on `POST /v1/mcp`, never a second configured server. The
...
whenever the mangrove was down. It is also OPTIONAL: unconfigured it registers
nothing, and no existing capability may acquire it as a dependency. See
`.specs/features/crab-mangrove-network/` and AD-029.
