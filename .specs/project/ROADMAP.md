...
- **crab-mangrove-network** (SLICES 1 AND 2 SHIPPED; the webapp tab is open) — a fifth submodule: a federated memory network over
  ...
  list. Delivered to harnesses as a `mangrove_` namespace on the MCP server the proxy **already**
  ...
  already carries the exact `(tenant, subscription, agent, user)` tuple the mangrove needs to
  ...
  open questions, the load-bearing ones being the runtime (Go recommended) and whether the mangrove
  ...
  The submodule exists and is public (`crab/crab-mangrove-network`, `MIT OR Apache-2.0`,
  ...
  **zero external dependencies** enforced in CI. It is **EXPERIMENTAL** and says so before
  ...
  See `.specs/features/crab-mangrove-network/`.
