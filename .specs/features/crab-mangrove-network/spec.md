# crab-mangrove-network — Specification
...
**And none of it is compulsory.** The mangrove is an optional part of the deployment: unconfigured, it
...
becomes urgent** — which it would be if the mangrove introduced a second, differently-scoped credential.
...
| Replacing or absorbing `internal/memgraph` | The mangrove *shares* the graph; private memory keeps exactly the shape and isolation it has today. |
...
**A4.** Each actor SHALL hold a signing keypair. Every activity the mangrove emits SHALL be signed by
...
and `cc` — the mangrove SHALL NOT invent an addressing mechanism beside them:
...
answered with `Reject`. The mangrove SHALL NOT maintain a membership list that can disagree with
...
**B5.** The mangrove SHALL NOT read a role from anything a caller supplies. Roles come from the decoded
`*mycelium.Profile`; where the mangrove needs a fact the profile does not carry, it SHALL be fetched
...
do not already reach.** For every activity, the mangrove SHALL compute the author's **reachable
...
reach**. The mangrove SHALL NOT silently drop unreachable addressees from the list and deliver to the
...
| Activity | Object | Meaning in the mangrove |
|---|---|---|
...
ActivityPub cannot un-deliver, and `Update` does not revoke replicas already sent. The mangrove SHALL
...
### FR-D — Delivery: one MCP server, a `mangrove_` namespace on the proxy

**D1.** Agents SHALL reach the mangrove **only** through the MCP server the proxy already hosts at
`POST /v1/mcp`. The number of MCP servers written into `.ganglion-config.json` and into picoclaw's
`config.json` SHALL remain exactly one. The mangrove submodule is a service on `zombie_net`; the proxy
...
**D2.** Every mangrove tool name SHALL be prefixed `mangrove_` and SHALL NOT collide with any of the 18
...
**D3.** The v1 tool surface SHALL be, at minimum: `mangrove_publish`, `mangrove_share`, `mangrove_unshare`,
`mangrove_timeline`, `mangrove_fetch`, `mangrove_follow`, `mangrove_unfollow`, `mangrove_ack`, `mangrove_endorse`,
`mangrove_flag`. Exact schemas are a design concern; the constraint here is that each maps to exactly
...
(`internal/mcpserver/tools.go:169-372`) plus three `schedule_*` (`tools.go:401-446`). The v1 mangrove
...
argument rather than shipped separately. `mangrove_ack`, `mangrove_endorse` and `mangrove_flag` are the obvious
candidates to fold, and `mangrove_follow`/`mangrove_unfollow` differ only by `Undo`. Design owns the final
...
**D5.** The mangrove SHALL degrade rather than fail. If the mangrove service is unreachable, the `mangrove_*`
...
**D6.** Whether `mangrove_*` tools are offered to picoclaw agents as well as ganglion agents in v1 is
...
**E1.** `mangrove_publish` and `mangrove_share` SHALL be gated tools, using the shipped approver contract
...
**F5.** The mangrove SHALL record, for every `Accept` and `Reject`, which account decided it and when.
...
**H1.** The mangrove's `README` SHALL state explicitly what is **not** protected: the proxy and the mangrove
...
`app/chat/sidebar-destinations.tsx:23-42`). The mangrove is scoped by subscription and tenant, spans
...
mangrove credential of its own, and SHALL NOT read the agent's MCP token.

**I8.** The tab SHALL render a reachable-but-empty mangrove and an unreachable mangrove as two different
...
The mangrove is an **optional** part of the deployment. A stack that never configures it SHALL behave
...
**J1 — unset is supported and is the safe default.** The mangrove SHALL be enabled by explicit
...
which `.claude/CLAUDE.md` names as *"the safe default, not an oversight"*. The mangrove SHALL follow it
rather than invent a third convention.

**J2 — unconfigured registers nothing.** With the mangrove unconfigured: no `mangrove_*` tool SHALL be
registered on `/v1/mcp`; no actor, keypair or collection SHALL be provisioned for any member; no
mangrove route SHALL be registered; and the webapp destination SHALL be **absent**, not present and
...
**J3 — the harness is untouched.** With the mangrove unconfigured, the number of MCP servers in
...
count at one whether the mangrove is on or off; J3 additionally requires that turning it on does not
change the *file* either, beyond what the façade already writes.

**J4 — nothing else depends on it.** No existing capability SHALL acquire the mangrove as a dependency.
`internal/memgraph`, the scheduler, projects, files, secrets, approvals and chat SHALL each work
exactly as they do now with the mangrove absent. In particular, **private memory SHALL NOT be routed
through the mangrove** — the mangrove reads and publishes *from* the graph; the graph does not read *through*
the mangrove.

**J5 — it can be turned off again.** Disabling a previously-enabled mangrove SHALL leave every local
memory graph and every workspace file intact and fully usable by the existing tools. No object SHALL
be stored in a form only the mangrove can read, and no local memory SHALL be replaced by a reference the
mangrove alone can resolve. Anything a member received and admitted into their own memory SHALL survive
as ordinary local memory.

**J6 — the failure of a share is not the failure of a turn.** Neither an unconfigured nor an
unreachable mangrove SHALL fail a turn, fail a boot, or block a tool the member was already using. The
worst outcome SHALL be a `mangrove_*` call returning a readable error — the same treatment a denied
...
| **AC-5** | With the gate configured and no approver answering, `mangrove_publish` is denied — fail closed, matching the shipped loop. |
| **AC-6** | The mangrove's `README` names the exposed metadata and every unprotected case in H1. |
| **AC-7** | A test asserts `.ganglion-config.json` still declares exactly one MCP server after the mangrove ships, and that the registered tool set has no name collision. |
...
| **AC-9** | With the mangrove service stopped, a ganglion container still boots and chats; only `mangrove_*` calls return errors. |
| **AC-10** | A human can list, in the webapp, an object their agent received and never mentioned in a conversation. |
| **AC-11** | The mangrove tab opens from the sidebar as a centre-pane destination, and `SECTION_ORDER` (`app/chat/workspace-sections.ts:17`) still holds exactly five — no sixth section appears beside a conversation. |
| **AC-12** | A hand-edited fragment carrying an unknown `v` still resolves to no destination; adding the mangrove value does not make `asDestination` accept arbitrary strings. |
| **AC-13** | A member with no governing role sees no pending-decisions reading at all — not an empty one. |
| **AC-14** | With the mangrove service stopped, the tab shows an unreachable state distinct from the empty state, and the rest of the app is unaffected. |
...
| **AC-21** | With the mangrove unconfigured: no `mangrove_*` tool is in the registered MCP tool set, `.ganglion-config.json` declares one server, `SECTION_ORDER` holds five, and no mangrove destination resolves. |
| **AC-22** | With the mangrove unconfigured, the webapp shows no mangrove destination at all — not a disabled or empty one. |
| **AC-23** | Enabling the mangrove and then disabling it leaves every memory graph and workspace file intact and usable by the existing tools; nothing is left as a mangrove-only reference. |
| **AC-24** | An unconfigured mangrove and an unreachable mangrove are distinguishable from the outside: the first registers no tool, the second returns a readable error from one. |
...
The recommendation is **Go**: the mangrove must decode the same `*mycelium.Profile` the proxy decodes,
...
**OQ-2 — Does the mangrove own its store, or reuse the proxy's?** The objects are memgraph nodes and
...
closed.** J4 requires the dependency to run one way — the mangrove reads *from* the graph, the graph
never reads *through* the mangrove — and J5 forbids storing any object in a form only the mangrove can read.
Together they rule out the mangrove holding the **canonical** copy of a shared memory object. What
...
**OQ-4 — Are `mangrove_*` tools offered to picoclaw agents in v1, or ganglion-only?** picoclaw's
...
**Met.** `crab/crab-mangrove-network` now exists as a public repository under
